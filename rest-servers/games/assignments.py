...
class Assignment:
    ...
    def list_by_player_id(sql_executor: database.SqlExecutor, player_id: int) -> typing.List[typing.Tuple[int, int]]:
        ...
        assignements_found = sql_executor.execute("SELECT * FROM assignements where player_id = ?", (player_id,), need_result=True)
        if not assignements_found:
            return []
        return assignements_found

    @staticmethod
    def list_by_tournament_id(sql_executor: database.SqlExecutor, tournament_id: int) -> typing.List[typing.Tuple[int, int]]:
        """ class lookup : finds the object in database from fame id """
        assignements_found = sql_executor.execute("SELECT * FROM assignements where tournament_id = ?", (tournament_id,), need_result=True)
        if not assignements_found:
            return []
        return assignements_found
    ...
    def inventory(sql_executor: database.SqlExecutor) -> typing.List[typing.Tuple[int, int, int]]:
        ...
        assignements_found = sql_executor.execute("SELECT * FROM assignements", need_result=True)
        if not assignements_found:
            return []
        return assignements_found
    ...
    def create_table(sql_executor: database.SqlExecutor) -> None:
        ...
        sql_executor.execute("DROP TABLE IF EXISTS assignements")
        sql_executor.execute("CREATE TABLE assignements (tournament_id INTEGER, player_id INTEGER)")
    ...
    def delete_database(self, sql_executor: database.SqlExecutor) -> None:
        ...
        sql_executor.execute("DELETE FROM assignements WHERE tournament_id = ? AND player_id = ?", (self._tournament_id, self._player_id))
