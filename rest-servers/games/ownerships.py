...
class Ownership:
    ...
    def list_by_game_id(sql_executor: database.SqlExecutor, game_id: int) -> typing.List[typing.Tuple[int, int, int]]:
        ...
        ownerships_found = sql_executor.execute("SELECT * FROM ownerships where game_id = ?", (game_id,), need_result=True)
    ...
    def create_table(sql_executor: database.SqlExecutor) -> None:
        ...
        sql_executor.execute("DROP TABLE IF EXISTS ownerships")
        sql_executor.execute("CREATE TABLE ownerships (game_id INTEGER, center_num INTEGER, role_num INTEGER)")
    ...
    def update_database(self, sql_executor: database.SqlExecutor) -> None:
        ...
        sql_executor.execute("INSERT OR REPLACE INTO ownerships (game_id, center_num, role_num) VALUES (?, ?, ?)", (self._game_id, self._center_num, self._role_num))

    def delete_database(self, sql_executor: database.SqlExecutor) -> None:
        """ Removes object from database """
        sql_executor.execute("DELETE FROM ownerships WHERE game_id = ? AND center_num = ? AND role_num = ?", (self._game_id, self._center_num, self._role_num))
