...
class CommunicationOrder:
    ...
    def list_by_game_id(game_id: int) -> typing.List[typing.Tuple[int, int, int, int, int, int]]:
        ...
        communication_orders_found = database.sql_execute("SELECT * FROM communication_orders where game_id = ?", (game_id,), need_result=True)
    ...
    def list_by_game_id_role_num(game_id: int, role_num: int) -> typing.List[typing.Tuple[int, int, int, int, int, int]]:
        """ class lookup : finds the object in database from fame id """
        communication_orders_found = database.sql_execute("SELECT * FROM communication_orders where game_id = ? and role_num = ?", (game_id, role_num), need_result=True)
        if not communication_orders_found:
            return []
        return communication_orders_found
    ...
    def create_table() -> None:
        ...
        database.sql_execute("DROP TABLE IF EXISTS communication_orders")
        database.sql_execute("CREATE TABLE communication_orders (game_id INTEGER, role_num INTEGER, order_type_num INTEGER, active_unit_zone_num INTEGER, passive_unit_zone_num INTEGER, destination_zone_num INTEGER)")
    ...
    def update_database(self) -> None:
        ...
        database.sql_execute("INSERT OR REPLACE INTO communication_orders (game_id, role_num, order_type_num, active_unit_zone_num, passive_unit_zone_num, destination_zone_num) VALUES (?, ?, ?, ?, ?, ?)", (self._game_id, self._role_num, self._order_type_num, self._active_unit_zone_num, self._passive_unit_zone_num, self._destination_zone_num))

    def delete_database(self) -> None:
        """ Removes object from database """
        database.sql_execute("DELETE FROM communication_orders WHERE game_id = ? AND role_num = ? AND active_unit_zone_num = ?", (self._game_id, self._role_num, self._active_unit_zone_num))
