...
class GameListRessource(flask_restful.Resource):  # type: ignore
    ...
    def get(self) -> typing.Tuple[typing.Dict[str, typing.Any], int]:  # pylint: disable=no-self-use
        ...
        data = {str(g.identifier): {'name': g.name, 'variant': g.variant, 'description': g.description, 'deadline': g.deadline, 'current_advancement': g.current_advancement, 'current_state': g.current_state, 'fast': g.fast, 'grace_duration': g.grace_duration, 'scoring': g.scoring, 'nopress': g.nopress, 'nomessage': g.nomessage} for g in games_list}
        return data, 200
...
class GameIncidentsRessource(flask_restful.Resource):  # type: ignore
    ...
    def get(self, game_id: int) -> typing.Tuple[typing.Dict[str, typing.List[typing.Tuple[int, int, int, float]]], int]:  # pylint: disable=no-self-use
        ...
        # check user has right to get status of incidents - must be game master or player in game - or admin
