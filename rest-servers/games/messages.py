...
class Message:
    ...
    def list_with_content_by_game_id(game_id: int) -> typing.List[typing.Tuple[int, int, int, int, int, contents.Content]]:
        ...
        messages_found = database.sql_execute("SELECT game_id, identifier, author_num, addressee_num, time_stamp, content_data FROM messages INNER JOIN contents ON contents.identifier = messages.content_id where game_id = ? ORDER BY time_stamp DESC", (game_id,), need_result=True)
