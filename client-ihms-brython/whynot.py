...
WHYNOT_CONTENT_TABLE = {
    ...
    "Par ce que cette brique est décomposée en un front end qui s'execute dans le navigateur et un back end qui est un serveur REST sur l'ordinateur loué pas l'association. Le gros de la lenteur vient du nombre des requêtes entre les deux que certaines pages nécessitent. Ce problème est accru dans les pays loins du serveur (il est en France). Par ailleurs, le développement du front end est en python compilé en javascript, aussi cela explique la lenteur relative de chargement de la page d'accueil.",
    ...
    "Pourquoi la résolution ne se déclenche pas automatiquement après la date limite, que les joueurs soient d'accord ou pas  ?":
    ...
    "Cela a été implémenté ainsi au départ, et tous les développements s'appuient sur le compte pour savoir si la requête vient d'un arbitre ou d'un joueur. Autoriser un même compte aurait beaucoup d'impacts sur toute la couche de vérification des autorisations.",

    "Pourquoi il faut un compte sur la brique sociale et un compte sur la brique jeu ?":
    "Nous avons pas encore trouvé ni le temps ni les compéteces pour le faire",
    ...
    "Pourquoi quand je passe mes ordres je suis obligé de cliquer sous le label alors que je pourrais plus simplement cliquer n'import où sur la zone ?":
    "Parce qu'un implémentation plus simmple a été réalisée qui détecte la position d'un clic non par rapport à son appartenance à un polygone, mais par sa proximité à un point central.",

    "Pourquoi les unités sont placées bizarrement dans leur zone sur la carte ?":
    ...
    "C'est prévu dans un avenir assez proche. Le principal obstacle sera la compréhension de la formule par le codeur !",
