...
FAQ_CONTENT_TABLE = {
    ...
    "Un administrateur peut modifier ses nouvelles. Il peut se substituer à un inscrit (donc cela implique qu'il puisse, s'il le souhaite, lire tous les messages privés et publics de toutes les parties). Il peut rectifier les paramètres fixes (en théorie) d'une partie. Il peut rectifier la position d'une partie. Il peut savoir les dates des dernières connexions et les connexions manquées (erreur de pseudo/mot de passe). Il gère la liste des créateurs et des modérateurs. Il peut (provisoirement) mettre à jouer le elo, la fiabilité et la régularité.",
