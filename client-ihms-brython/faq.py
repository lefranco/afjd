...
FAQ_CONTENT_TABLE = {
    ...
    "Un administrateur peut modifier ses nouvelles. Il peut se substituer à un inscrit (donc cela implique qu'il puisse, s'il le souhaite, lire tous les négociations et déclarations de toutes les parties). Il peut rectifier les paramètres fixes (en théorie) et la position d'une partie. Il peut savoir les dates des dernières connexions réussies et manquées (erreur de pseudo/mot de passe). Il gère la liste des créateurs et des modérateurs. Il peut (provisoirement) mettre à jour le elo, la fiabilité et la régularité. Il peut voir les comptes oisifs et les comptes qui n'ont pas confirmé leur adresse courriel.",
