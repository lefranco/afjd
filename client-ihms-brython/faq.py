...
FAQ_CONTENT_TABLE = {
    ...
    "C'est tout ?":
    "Non. Premièrement : un arbitre peut le faire à sa place pour éviter que la partie ne s'éternise, avec les mêmes conséquences. Deuxièmement un “d'accord pour résoudre à la date limite“ exprimé après la date limite devient un “d'accord pour résoudre maintenant“ avec les mêmes conséquences. Troisièmement : un petit automate vient regarder les parties dont la date limite est passée toutes les heures. Il se charge de soigeusement transformer tout “d'accord pour résoudre à la date limite“ en “d'accord pour résoudre maintenant“ toujours avec les mêmes conséquences.",
    ...
    "Non ! Les messages sont certes privés entre émetteur et destinataire(s). Les presses sont certes privées dans la partie et avec le modérateur du site (qui peut les lire et en écrire). Dans les deux cas le contenu doit respecter la charte. L'administrateur peut sur demande lire un message pour vérifier. Contenu inaproprié ? Déclarez un incident ! (reperez le message par son id)",
    ...
    "Ces deux paramètres ont une version “pour la partie“ et une version “en ce moment“ La première est celle entrée par le créateur de la partie. La deuxième est celle qui s'applique actuellement, elle est initialisée à la valeur de la première. A la fin de la partie, la deuxième est effacée. L'arbitre peut également jouer avec l'autorisation des messages publics et privés pendant la partie. Les exportations de partie (pour savoir comment elle a été jouée) se baseront sur la première. Une partie se joue en principe selon les modalités de la première, mais à la fin de la partie, il faut pouvoir débriefer, d'où l'intérêt de la seconde...",
    ...
    "Faire venir ou partir les joueurs de la partie se réalise par contre dans le editer parties / déplacer des joueurs. Un joueur peut se mettre ou se retirer d'une partie, un arbitre peut mettre un joueur ou le retirer d'une partie. Pour un joueur, il y a trois manière de rejoindre une partie : par le menu “Parties“ sous menu “Appariement“ (qui gère tous les cas), par le menu opportunités (qui liste les parties qui recrutent), par un hyper lien que quelqu'un vous aura envoyé s(pour vous inviter).",
