...
def my_games(state):
    ...
    for game_id_str, data in sorted(games_dict.items(), key=lambda g: g[1]['name']):
        ...
        report += f"considering game {data['name']}"
        before = time.time()
        ...
        after_el = time.time()
        elapsed = after_el - before_el
        report += f"get_role_allocated_to_player={elapsed}"
        ...
        after_el = time.time()
        elapsed = after_el - before_el
        report += f"get_roles_submitted_orders={elapsed}"
        ...
        for field in fields:
            ...
            if field == 'new_declarations':
                ...
                if role_id is not None:
                    ...
                    after_el = time.time()
                    elapsed = after_el - before_el
                    report += f"last_visit_load/last_game_declaration={elapsed}"
            ...
            if field == 'new_messages':
                ...
                if role_id is not None:
                    ...
                    after_el = time.time()
                    elapsed = after_el - before_el
                    report += f"last_visit_load/last_game_message={elapsed}"
