...
def game_master():
    ...
    def send_recall_orders_email_callback(ev, role_id):  # pylint: disable=invalid-name
        ...
        body += "\n"
        body += "Pour la première saison, cela peut être immédiat car on considère que le nouveau joueur n'a pas/plus l'intention de jouer."
        body += "\n"
    ...
    def send_welcome_email_callback(ev, role_id):  # pylint: disable=invalid-name
        ...
        body += "\n"
        body += "Conseil : ne tardez pas trop à entrer vos ordres. En effet, certains arbitres remplacent un joueur en retard au premier tour."
        body += "\n"
        body += "Ce serait dommage de, par une petit négligence, perdre cette oppotunité de jouer !"
        body += "\n"
        body += "Pour se rendre directement sur la partie :\n"
        body += f"{config.SITE_ADDRESS}?game={play_low.GAME}"
