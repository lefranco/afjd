...
class Event:
    ...
    def __init__(self, identifier: int, name: str, start_date: str, start_hour: str, end_date: str, location: str, priority: int, description: str, summary: str, manager_id: int) -> None:
        ...
        assert isinstance(description, str), "description must be a str"
        self._description = description

        assert isinstance(summary, str), "summary must be a str"
    ...
    def delete_registrations(self, sql_executor: database.SqlExecutor) -> None:
        ...
        for event_id, player_id, date_, approved, comment in registrations_list:
            registration = registrations.Registration(event_id, player_id, date_, approved, comment)
...
def convert_event(buffer: bytes) -> Event:
    ...
    priority = int(tab[6].decode())

    compressed_description = tab[7].decode()
    description = database.uncompress_text(compressed_description)

    compressed_summary = tab[8].decode()
    summary = database.uncompress_text(compressed_summary)

    manager_id = int(tab[9].decode())
    event = Event(identifier, name, start_date, start_hour, end_date, location, priority, description, summary, manager_id)
    return event
