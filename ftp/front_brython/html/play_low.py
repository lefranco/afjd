...
def stack_last_agreements_button(button_container, content_container, insert_before, disable_callback, disabled):
    ...
    input_last_agreements = html.INPUT(type="submit", value="Afficher en dessous les derniers arrangements", Class='btn-inside')
...
def stack_position_and_my_orders(button_container, content_container, insert_before):
    ...
    input_my_orders = html.INPUT(type="submit", value="Afficher en dessous la position et mes ordres", Class='btn-inside')
...
def stack_communications_orders_button(button_container, content_container, insert_before, disable_callback, disabled):
    ...
    input_communications_orders = html.INPUT(type="submit", value="Afficher en dessous mes ordres de com'", Class='btn-inside')
...
def stack_role_builds(frame):
    ...
    if ROLE_ID != 0:
        ...
        available_info = f" et {nb_free_centers} emplacement(s) inoccupé(s)" if nb_ownerships > nb_units else ""
        frame <= html.DIV(f"Vous avez {nb_ownerships} centre(s) pour {nb_units} unité(s){available_info}. Vous {'construisez' if nb_builds >= 0 else 'détruisez'} donc {abs(nb_builds)} fois.", Class='note')
