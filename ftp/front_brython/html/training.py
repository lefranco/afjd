...
def slide_just_display(table_of_contents):
    ...
    # advertise
    url = f"{config.SITE_ADDRESS}?sequence={SEQUENCE_NAME}"
    ...
    def callback_canvas_click(event):
        ...
        if automaton_state is AutomatonStateEnum.SELECT_ACTIVE_STATE:
            ...
            if selected_active_unit is None or selected_active_unit.role != VARIANT_DATA.roles[ROLE_ID]:
                ...
                if advancement_season is mapping.SeasonEnum.ADJUST_SEASON:
                    order = mapping.Order(POSITION_DATA, selected_order_type, selected_active_unit, None, None, False)
                    orders_data.insert_order(order)
        ...
        if automaton_state is AutomatonStateEnum.SELECT_DESTINATION_STATE:
            ...
            if advancement_season is mapping.SeasonEnum.ADJUST_SEASON:
                selected_build_zone = VARIANT_DATA.closest_zone(pos, selected_build_unit_type)
            ...
            if selected_order_type is mapping.OrderTypeEnum.BUILD_ORDER:
                ...
                if center is None and emergency_center is None:
                    ...
                    # becomes tricky
                    accepted = True
                    deducted_role = POSITION_DATA.owner_table[center].role if center else emergency_center.role
