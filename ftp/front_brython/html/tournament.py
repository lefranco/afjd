...
OPTIONS = {
    ...
    'Les tournois du site': "Liste complète des tournois sur le site et statistiques sur leur fréquentation",
}
...
def show_informations():
    ...
    MY_SUB_PANEL <= html.H3(f"Informations publiques tournoi {tournament_name}")
    ...
    for player_id_str, times in sorted(players_tournament_dict.items(), key=lambda t: id2pseudo[int(t[0])].upper()):

        row = html.TR()

        col = html.TD(id2pseudo[int(player_id_str)])
        row <= col
    ...
    MY_SUB_PANEL <= html.H4("Retards sur le tournoi")
...
def handle_tournament():
    ...
    # title 1
    MY_SUB_PANEL <= html.H3(f"Gérer le tournoi {tournament_name}")

    # title 1
    MY_SUB_PANEL <= html.H4("Mettre dans ou enlever des parties du tournoi")
    ...
    MY_SUB_PANEL <= html.H4("Supprimer le tournoi")
...
def show_tournaments_data():
    ...
    fields = ['tournament', 'creator', 'first_start_time', 'last_start_time', 'first_end_time', 'last_end_time', 'affluence', 'games']

    # header
    thead = html.THEAD()
    for field in fields:
        field_fr = {'tournament': 'tournoi', 'creator': 'créateur', 'first_start_time': 'première première résolution', 'last_start_time': 'dernière première résolution', 'first_end_time': 'première dernière résolution', 'last_end_time': 'dernière dernière résolution', 'affluence': 'affluence', 'games': 'parties'}[field]
        col = html.TD(field_fr)
        thead <= col
