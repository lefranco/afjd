	...
	while (ajustements < souhaites) {
		...
		for (t = UNITE.t; t < UNITE.t + UNITE.n; t++) {
			...
			if (el == elrec) {
				...
				/* Si c'est une flotte et l'autre une arm�e il passe devant */
				if (t->typeunite == FLOTTE && trec->typeunite == ARMEE) {
					trec = t;
					continue;
				}

				/* S'il est avant dans l'ordre alphab�tique il passe devant */
...
void verifmouvements(void) {
	...
	/* Recherche des doublons distincts ou �gaux : on s'arrete au premier */
	for (p = MOUVEMENT.t; p < MOUVEMENT.t + MOUVEMENT.n - 1; p++)
		for (q = p + 1; q < MOUVEMENT.t + MOUVEMENT.n; q++)
...
void verifajustements(void) {
	...
	/* Recherche des doublons distincts ou �gaux construction : on s'arrete au premier */
	for (u = AJUSTEMENT.t; u < AJUSTEMENT.t + AJUSTEMENT.n - 1; u++)
		for (v = u + 1; v < AJUSTEMENT.t + AJUSTEMENT.n; v++)
	...
	/* Recherche des doublons distincts ou �gaux suppression : on s'arrete au premier */
	for (u = AJUSTEMENT.t; u < AJUSTEMENT.t + AJUSTEMENT.n - 1; u++)
		for (v = u + 1; v < AJUSTEMENT.t + AJUSTEMENT.n; v++)
	...
	for (r = PAYS.t; r < PAYS.t + PAYS.n; r++) {
		...
		/* On affiche un r�sum� complet de la situation */
		...
		if (souhaites < 0 && abs(ajustements) < abs(souhaites)) {
			...
			cherchechaine(__FILE__, 27, buf, 1, bufn1); /*"Suppression de %1 unit�(s) attendue, moins de suppressions qu'attendu"*/
			erreurverif(r, COHERENCE, buf);
		}
		if (souhaites < 0 && abs(ajustements) > abs(souhaites)) {
			(void) sprintf(bufn1, "%d", abs(souhaites));
			cherchechaine(__FILE__, 28, buf, 1, bufn1); /*"Suppression de %1 unit�(s) attendue, plus de suppressions qu'attendu"*/
			erreurverif(r, COHERENCE, buf);
		}
		if (souhaites > 0 && ajustements > souhaites) {
			(void) sprintf(bufn1, "%d", souhaites);
			cherchechaine(__FILE__, 29, buf, 1, bufn1); /*"Construction de %1 unit�(s) attendue, plus de constructions qu'attendu"*/
			erreurverif(r, COHERENCE, buf);
		}
		...
		if (souhaites > 0 && ajustements < souhaites) {
			...
			if (*p == EOS) { /* Pas parmi les exclus */
				...
				informer(buf);
				(void) sprintf(bufn1, "%d", souhaites);
				cherchechaine(__FILE__, 30, buf, 1, bufn1); /*"Construction de %1 unit�(s) attendue, moins de constructions qu'attendu"*/
				avertir(buf);
			}
		}
	}
