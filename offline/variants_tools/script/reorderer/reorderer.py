...
"""
Reorder everythng to have a nice alphabetical order
"""
...
def reorder(json_variant_data: typing.Dict[str, typing.Any], json_parameters_data: typing.Dict[str, typing.Any]) -> None:
    ...
    # regions
    json_variant_data['regions'].insert(INSERT_POSITION - 1, -1)

    # centers
    json_variant_data['centers'] = list(map(lambda n: n + 1 if n >= INSERT_POSITION else n, json_variant_data['centers']))  # type: ignore

    # coastal_zones
    json_variant_data['coastal_zones'] = list(map(lambda ll: [ll[0], ll[1] + 1] if ll[1] >= INSERT_POSITION else ll, json_variant_data['coastal_zones']))  # type: ignore
